    ...
    return (
        <div className={`group my-2 border border-gray-200 rounded-md col-span-${colSpan} hover:-translate-y-1 hover:shadow-lg transition-all duration-400`}>
            <div className="pt-4 bg-gray-50 px-4 flex flex-row flex-wrap">
                {tech.map((tech, index) => (
                    <div className="bg-gray-100 border border-gray-100 text-gray-700 font-thin py-1 px-2 text-xs rounded-lg mx-1" key={index}>{tech}</div>
                ))}
            </div>
            ...
            <h4 className="px-4 mt-6 md:mt-12 group-hover:mt-11 text-lg font-bold transition-all duration-300">{name}</h4>
